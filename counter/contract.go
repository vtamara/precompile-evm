...
const (
	...
	GetCounterGasCost       uint64 = 7 /* SET A GAS COST HERE */
	IncrementCounterGasCost uint64 = 7 /* SET A GAS COST HERE */
	SetCounterGasCost       uint64 = 7 /* SET A GAS COST HERE */
)
...
var (
	...
	// Added
  storageKeyHash = common.BytesToHash([]byte("counterKey"))   
)

// sets the value of the counter in "counterKey" in the contract storage.
func StoreCounterValue(stateDB contract.StateDB, newValue *big.Int) {
...
func getCounter(accessibleState contract.AccessibleState, caller common.Address, addr common.Address, input []byte, suppliedGas uint64, readOnly bool) (ret []byte, remainingGas uint64, err error) {
	...
	o := GetCounterValue(currentState)
	output = &o

	packedOutput, err := PackGetCounterOutput(output)
	if err != nil {
...
func incrementCounter(accessibleState contract.AccessibleState, caller common.Address, addr common.Address, input []byte, suppliedGas uint64, readOnly bool) (ret []byte, remainingGas uint64, err error) {
	...
	StoreCounterValue(currentState, big.NewInt(0).Add(&value, big.NewInt(1)))
